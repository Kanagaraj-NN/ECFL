...
def extract_line_feature(input_file, output_file, project_name, bug_id, orig_buggy_commit_id):
    ...
    for susp_line in sorted_susp_lines:
        ...
        if line_number in git_blame_lines:
            ...
            if project_name == 'Chart':
                line_recency = extract_line_recency_svn_Chart(blame_line[0])
            else:
                line_recency = extract_line_recency(blame_line[0])
            add_susp_data_to_file(output_file,susp_line_id,suspiciousness,line_recency)
...
def extract_line_recency(blame_line):
...
def extract_line_recency_svn_Chart(blame_line):
    """Parses the line and extract the line length
...
if __name__ == '__main__':
    ...
    Command to run:
    --------------
    # python3.6 step1b_extract_LineRecency_feature.py
    ...
    print(f"\nSCRIPT NAME: step1b_extract_LineRecency_feature.py")
