...
def extract_line_feature(input_file, output_file, project_name, bug_id, orig_buggy_commit_id):
    ...
    for susp_line in sorted_susp_lines:
        ...
        if line_number in git_blame_lines:
            ...
            if project_name == 'Chart':
                LineLength = extract_line_length_svn_Chart(blame_line[0])
            else:
                LineLength = extract_line_length(blame_line[0])
            add_susp_data_to_file(output_file,susp_line_id,suspiciousness,LineLength)
...
def add_susp_data_to_file(output_file, susp_line_id, suspiciousness, LineLength):
    ...
    with open(output_file, mode="a", encoding="utf-8") as myFile:
        myFile.write(f"{susp_line_id},{suspiciousness},{LineLength}\n")
...
def extract_line_length(blame_line):
    """Parses the line and extract the line length
    ...
    Return:
    -------
    line_len_counter: int (Number of characters in a line)
    ...
    return line_len_counter


def extract_line_length_svn_Chart(blame_line):
    """Parses the line and extract the line length

    Parameters:
    ----------
    line: str (line from the svn blame output file)
...
if __name__ == '__main__':
    ...
    for project, bugs in zip(PROJECTS, PROJECT_BUGS):
        ...
        for bug in bugs:
            ...
            output_csv = f"{project}-{bug}-{FL_TECHNIQUE}-sorted-susp-LineLength.csv"
    ...
    print(f"\nSCRIPT NAME: step1a_extract_LineLength_feature.py")
