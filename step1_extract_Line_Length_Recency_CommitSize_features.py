...
FL_TECHNIQUE = 'ML_combineFL'  # 'ML_combineFL', 'dstar2' or 'combineFastestFL', 'combineFL_nearest' (Fault Localization baseline)
...
def extract_line_feature(input_file, output_file, project_name, bug_id, orig_buggy_commit_id):
    """Extract all 3 features (LineLength, LineRecency, LintCommitSize) for every suspicious line
    ...
    with open(output_file, mode="a", encoding="utf-8") as myFile:
        myFile.write(f"susp_line,suspiciousness,LineLength,LineRecency,LineCommitSize\n")
    ...
    # Loop through every line, do line mapping and extract line features
    for susp_line in sorted_susp_lines:
        ...
        if line_number in git_blame_lines:
            ...
            if project_name == 'Chart':
                line_length, line_recency, line_commit_id = extract_line_features_svn_Chart(blame_line[0])
                ...
                line_length, line_recency, line_commit_id = extract_line_features(blame_line[0])
                if line_commit_id in commit_id_size_table:
                    line_commit_size = commit_id_size_table[line_commit_id]
                else:
            ...
            add_susp_data_to_file(output_file,susp_line_id,suspiciousness,line_length,line_recency,line_commit_size)
...
def add_susp_data_to_file(output_file, susp_line_id, suspiciousness, line_length, line_recency, line_commit_size):
    """Adds all 3 feature to the sorted suspiciousness file
    ...
    line_length: int
    line_recency: str (date)
    line_commit_size: int

    """

    with open(output_file, mode="a", encoding="utf-8") as myFile:
        myFile.write(f"{susp_line_id},{suspiciousness},{line_length},{line_recency},{line_commit_size}\n")
...
def extract_line_features(blame_line):
    ...
    return line_len_counter, date, line_commit_id
...
def extract_line_features_svn_Chart(blame_line):
    """Parses the line and extract the line commit id
    ...
    return line_length, date, line_commit_id
...
if __name__ == '__main__':
    ...
    output_directory = f"{WORKING_DIR}/output_{FEATURE}/step1_{FEATURE}"
