...
if __name__ == '__main__':
    ...
    input_directory = f"{WORKING_DIR}/fault-localization.cs.washington.edu/line_suspiciousness_{FL_TECHNIQUE}"
    if not os.path.exists(input_directory):
        raise ValueError(f"Input directory {input_directory} is not found. "
                         "Plz run the previous step. "
                         "Then re-run this step")

    output_directory = f"{WORKING_DIR}/fault-localization.cs.washington.edu/line_suspiciousness_sorted_{FL_TECHNIQUE}"
