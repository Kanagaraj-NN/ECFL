...
def compute_different_weights(sorted_susp_lines, susp_weight, feature_weight, output_file):
    ...
    output_file_temp = output_file + f"-{susp_weight}-{feature_weight}.csv"

    for susp_line in sorted_susp_lines:
        susp = susp_line[-4].strip()
        feature = susp_line[-2].strip()
    ...
    with open(output_file, mode="a", encoding="utf-8") as myFile:
        myFile.write(f"susp_line_id,suspiciousness,line_length,line_recency,line_commit_size,normalized_suspiciousness,"
                     f"normalized_line_length,normalized_line_recency,normalized_line_commit_size,weighted_susp_score\n")
...
if __name__ == '__main__':
    ...
    output_directory = f"{WORKING_DIR}/output_{FEATURE}/step3_{FEATURE}_wt_{SINGLE_FEATURE}"
    ...
    for project, bugs in zip(PROJECTS, PROJECT_BUGS):
        for bug in bugs:
            ...
            output_csv = f"{project}-{bug}-{FL_TECHNIQUE}-sorted-susp-{FEATURE}-{SINGLE_FEATURE}-weighted"
    ...
    print(f"WEIGHT PAIRS SELECTED: [(Susp_weight, Feature_weight)]: {WEIGHT_PAIRS}")
