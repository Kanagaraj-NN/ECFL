...
def convert_csv_to_json_format(input_file, output_file):
    ...
    with open(output_file, mode="a", encoding="utf-8") as myFile:
        for i, susp_line in enumerate(sorted_susp_lines):
            ...
            line_length = susp_line[-3]
            line_recency = susp_line[-2]
            line_commit_size = susp_line[-1]
            myFile.write(f'\t\t"{line_path}:{line_id_number}": ')
            myFile.write("{\n")
            myFile.write(f'\t\t\t"LineLength": {line_length},\n')
            myFile.write(f'\t\t\t"LineRecency": {line_recency},\n')
            myFile.write(f'\t\t\t"LineCommitSize": {line_commit_size}\n')
...
if __name__ == '__main__':
    ...
    output_file_name = f"{FEATURE}_top_100.json"
    ...
    print(f"\nSCRIPT NAME: convert_all_3_features_csv_to_json_format.py")
