...
PassFailTally = collections.namedtuple('PassFailTally',
...
def parse_test_summary(line, n_elements):
    ...
    if len(coverages) != n_elements:
        raise ValueError(
            "expected {expected} elements in each row, got {actual} in {line!r}".format(expected=n_elements,
                                                                                        actual=len(coverages),
                                                                                        line=line))
    return TestSummary(
        triggering=(sign == '-'),
...
if __name__ == '__main__':
    ...
    input_directory = "%s/fault-localization.cs.washington.edu/coverage" % WORKING_DIR
    if not os.path.exists(input_directory):
        raise ValueError("Input directory %s is not found. "
                         "Plz run the previous step. "
                         "Then re-run this step" % input_directory)

    output_directory = "%s/fault-localization.cs.washington.edu/stmt_suspiciousness" % WORKING_DIR
    ...
    coverage_files = sorted(glob(os.path.join(input_directory, '*coverage')))
    spectra_files = sorted(glob(os.path.join(input_directory, '*spectra')))
    ...
    for coverage_file, spectra_file in zip(coverage_files, spectra_files):
        generate_suspiciousness(FL_TECHNIQUE, coverage_file, spectra_file, output_directory)
    ...
    print("\nSCRIPT NAME: 3_compute_suspiciousness.py")
